...
tf.app.flags.DEFINE_string('data_dir', r'D:/datasets/data_cifar10', 'Path to the cifar10 data directory')
...
def inputs(eval_data):
    ...
    if FLAGS.use_fp16:
        images = tf.cast(images, tf.float16)
        labels = tf.cast(labels, tf.flaot16)
    return images, labels
...
def inference(images):
    ...
    # Softmax
    with tf.variable_scope('softmat_linear') as scope:
...
def maybe_download_and_extract():
    ...
    if not os.path.exists(filepath):
        def _progress(count, block_size, total_size):
            sys.stdout.write('\r>> Downloading {} {:.1f}%'.format(filename, float(count * block_size) / float(total_size) * 100))
            sys.stdout.flush()
